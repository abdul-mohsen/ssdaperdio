...
public class Frame extends JFrame {
	...
	int postion = 0;
	// here
	int N = 20, last = 0, score = 0, lives = 3, speed = (int) (6 * proX), ThreadSpeed = 8; // Right=1,Left=-1;
	...
	public Frame() {
		...
		// audio
		try {
			BGM1 = new SimpleAudioPlayer("background music2.wav");
			BGM1.play();
		...
		a[13] = new obj(6000, 900, 10, 10);
		a[14] = new obj(6500, 900, 10, 10);
		a[15] = new obj(7000, 900, 10, 10);
		a[16] = new obj(7500, 900, 10, 10);
		a[17] = new obj(8000, 900, 10, 10);
		a[18] = new obj(8500, 900, 10, 10);
		a[19] = new obj(6000, 200, 3000, 500);
		for (int i = 0; i < N; i++) {
			a[i].setX((int) (a[i].getX() * proX));
			a[i].setLH((int) Math.ceil(a[i].getLH() * proX));
			a[i].setY((int) (a[i].getY() * proY));
			a[i].setLV((int) Math.ceil(a[i].getLV() * proY));
		}
		// here
		// enemy thread

		Enemy = new Enemy[2];
		Enemy[0] = new Enemy(1200, 901, true, true, Color.pink,0,0);
		Enemy[1] = new Enemy(4400, 901, false, false, Color.GREEN,4400,5000);
		Enemy[0].start();
		//Enemy[1].start();
		// end
	...
	class Enemy extends Thread {
		...
		int width, hight,LR=0,LL=0;
		boolean Dead = false,moving=false;
		Random x = new Random();
		int tempRand;
		boolean fall, killable;// can fall form ground-----can be killed
		Color C;

		public Enemy(int posx, int posy, boolean fall, boolean killable, Color color,int LL,int LR) {
		...
		public void run() {
			...
			while (!Dead) {
				...
				while (!Dead && (i < Math.abs(tempRand))) {
					...
					if (!(this.posy > Character.getPosy() + Character.getHight()
							|| this.posy + this.width < Character.getPosy())
							&& !(this.posx + this.width < Character.getPosx()
									|| this.posx > Character.getPosx() + Character.getWidth())) {
						die();
					}
				...
				while (checkJump(Character.posx, Character.posy, Character.width, Character.hight)) {
					...
					for (int E = 0; E < NE; E++)
						if (!Enemy[E].Dead && Character.posy + Character.hight < Enemy[E].posy
								...
								&& !(Character.posx > Enemy[E].posx + Enemy[E].width
										|| Character.posx + Character.width < Enemy[E].posx)) {
							d = 0;
							if (Enemy[E].killable) {
								Enemy[E].Dead = true;
								Enemy[E].moving=false;
...
//							postion -= 1;
					...
					for(int i=0;i<NE;i++){
						if(Enemy[i].posx<1980&&!Enemy[i].Dead){
							if(!Enemy[i].isAlive()){
								Enemy[i].moving=true;
								Enemy[i].start();
							}
							else if(!Enemy[i].moving){
								Enemy[i].moving=true;
								Enemy[i].resume();}
							}
						}
					...
					if (checkJump(Character.posx, Character.posy + 1, Character.width, Character.hight) && fall
							&& d < con) {
						d = con;
						z6 = new Jump();
						z6.start();
...
//			if (!(x + 1 >  || x + w - 1 < a[i].getX() + 1)
//					&& !(y + 1 > a[i].getY() + a[i].getLV() || y + h - 2 < a[i].getY()))
			if(y <= Y1 && y + h >= a[i].getY() &&
					X1 <= x && x <= X1 + speed - 3) {
